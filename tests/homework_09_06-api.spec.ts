...
test('Get order with correct parameters username and password - answer 200 OK', async ({ request }) => {
  const response = await request.get('https://backend.tallinn-learning.ee/test-orders?username=Germans&password=asd')
  expect(response.status()).toBe(StatusCodes.OK)
})

test('Get order with no parameters username and password - Internal Server Errorr ', async ({ request }) => {
...
test('Get order with only 1 parameter  username - Internal Server Errorr ', async ({ request }) => {
  const response = await request.get('https://backend.tallinn-learning.ee/test-orders?username=Germans')
  expect(response.status()).toBe(StatusCodes.INTERNAL_SERVER_ERROR)
})
